...
- **摆动恢复 Stage 3 已完成，当前 NTU2P 主线最佳 checkpoint 为 `save/forecasting/ntu120_label/ntu2p_residual_refiner_xyz_artic_s3_s2_5_dct1_root01_s0_20000/model000020000.pt`**：s2_5 配置 3 seed @5000 mpjpe 0.1911 ± 0.0040、dct_mid 0.59 ± 0.10，全部过 gate；20000 step 达 mpjpe 0.1789（相对 base −21.0%、copy-last −35.6%），摆动指标保持（low 0.595 / mid 0.513 / high 0.0098，frozen 0.123）。误差随 horizon 分解：root 误差近似线性增长不饱和（2.5 s 时 0.179，为长程偏差主因，可修：root 轨迹 DCT 头、新损失下重测 inter 权重）；局部姿态误差 1.0–1.5 s 后饱和于 0.15–0.19（本质多模态，不可由确定性模型修复，交 Track B residual diffusion 并对 ≥1.5 s 改报合理性指标）；不要用加大 long/final 权重压长程 L2。结果与 horizon 曲线见 `docs/ai/context/20260902-205844-ntu2p-articulation-recovery-stage3-result.md`。
- NTU2P residual refiner 训练步数预算（据 20k 长训平台统计：8k–20k mpjpe 0.1823 ± 0.0023，train/val gap 扩大，膝点 6000–8000）：**消融/筛选 5000 step，最终数字 10000 step × 3 seed，取终点 checkpoint 报均值 ± 标准差，不在 val 上挑最好**；不再使用 20000。可选低成本改进：学习率余弦衰减或权重 EMA（需先实现并做等价性测试）。设计见 `docs/ai/context/20260902-212338-ntu2p-training-step-budget-design.md`。
